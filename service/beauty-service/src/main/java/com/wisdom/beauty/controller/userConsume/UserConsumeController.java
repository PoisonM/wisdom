...
//	@LoginRequired
    ...
    @ResponseBody
    @Transactional
    ResponseDTO<String> userConsumeOperation(@RequestBody List<ExtShopUserConsumeRecordDTO> extShopUserConsumeRecordDTOS) {

        long currentTimeMillis = System.currentTimeMillis();
        logger.info("用户消费操作传入参数={}", "shopUserConsumeRecordDTOS = [" + extShopUserConsumeRecordDTOS + "]");

        ResponseDTO<String> responseDTO = new ResponseDTO<>();

        if (CommonUtils.objectIsNotEmpty(extShopUserConsumeRecordDTOS)) {
            logger.debug("用户消费操作传入参数为空, {}", "shopUserConsumeRecordDTOS = [" + extShopUserConsumeRecordDTOS + "]");
            return null;
        }
        //生成唯一的交易流水号
        String transactionCodeNumber = getTransactionCodeNumber();
